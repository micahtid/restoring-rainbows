...
/**
 * Interactive map component displaying branch locations using Leaflet
 * Handles SSR issues with dynamic imports and client-side rendering
 */
const BranchMap: React.FC<BranchMapProps> = ({ branches }) => {
  ...
  // PERFORMANCE: Memoize marker click handler to prevent unnecessary re-renders
    ...
    const loadLeaflet = async () => {
      if (typeof window !== 'undefined') {
        try {
          ...
          // Create custom red dot icon for branch markers...
  ...
  // PERFORMANCE: Memoize markers to prevent unnecessary re-renders when branches don't change...
  const markers = useMemo(() => {
    if (!branches || !redDotIcon) return null;
  ...
  }, [branches, redDotIcon, handleMarkerClick]);
  ...
  return (
    <div className='max-w-max w-full mx-auto mt-10 mb-24 h-[500px] px-4 fade-in-animation'>
      ...
      <DynamicMapContainer
        center={[51.509865, -0.118092]} // London coordinates as default center
